...
{
public :
	static const int num_arcs=128;

	dicc_arco	*arcs[num_arcs];
	autor		*a;

	dicc_nodo()
	{
		for( int i=0; i < num_arcs; i++ ) arcs[i]=NULL;
		a=NULL;
	}
	dicc_nodo( autor *a )
	{
	...
	inline dicc_arco * cerca_arco( char ch )
	{
		return arcs[(int)ch];
	}
...
public :
	...
	void mostra( dicc_nodo *nodo )
	{
		...
		for( int i=0; i < nodo->num_arcs; i++ )
			if ( nodo->arcs[i] != NULL )
				mostra( nodo->arcs[i]->nodo );
	}
};

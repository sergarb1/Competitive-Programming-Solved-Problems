...
{ 
  ...
  #ifndef ONLINE_JUDGE 
          close (0); open ("11115.in", O_RDONLY); 
          close (1); open ("11115.out", O_WRONLY | O_CREAT, 0600); 
  #endif 
  long long int x,y;
  while(cin >> x >> y)
  {
            if(x==0 && y==0)
                    return 0;
            int i,j;
            bignum res,res2,cons;
            int_to_bignum(1,&res);
            int_to_bignum(x,&cons);
            for(i=0;i<y;i++)
            {
                if(i%2==0)
                          multiply_bignum(&res,&cons,&res2);
                else
                          multiply_bignum(&res2,&cons,&res);
                
            }
            if(i%2==0)
                      cout <<res.digits<< endl;
            else
                      cout <<res2.digits<< endl;

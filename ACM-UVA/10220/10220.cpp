...
long long int resultado(bignum* m)
{
    long long int r=0;
    
    for(int i=0;i<m->lastdigit;i++)
    {
            r+=m->digits[i]-'0';
    }
    return r;
...
{ 
  ...
  int n;
  #ifndef ONLINE_JUDGE 
          close (0); open ("10220.in", O_RDONLY); 
          close (1); open ("10220.out", O_WRONLY | O_CREAT, 0600); 
  #endif 
  bignum num1;
  bignum num2;
  bignum num3;
  while(cin >> n)
  {
            ...
            int_to_bignum(1,&num2);
            for(i=2;i<=n;i++)
            {
                 int_to_bignum(i,&num1);
                 if(i%2==0)
                 {
                 ...
                 else
                 {
                        ...
                        multiply_bignum(&num1,&num3,&num2);
                 }
            }
            if(i%2==0)
            {
                      long long int res=resultado(&num2);
                      cout << res << endl;
            }
            else
            {
                      long long int res=resultado(&num3);
                      cout << res << endl;
            }
  }
  return 0;

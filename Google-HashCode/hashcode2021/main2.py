...
def calcularSolucion():
    ...
    for i in range(len(listaCalles)):
        ...
        if miCalle[2] not in dicCallesTiempo or dicCallesTiempo[miCalle[2]]<1:
            tiempo=1
            continue
        else:


            if not miCalle[1] in res or miCalle[3]>300:
                res[miCalle[1]] = []
            if(dicCallesTiempo[miCalle[2]]>80):
                tiempo=100
            if(dicCallesTiempo[miCalle[2]]>50):
                tiempo=32
            else:
                tiempo=1
